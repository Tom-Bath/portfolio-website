...
I joined the project as the team began navigating the transition from a limited proof-of-concept to a serious product offering, focusing on creating a refined and accessible user experience.
...
- **Material and Information Flow Charts (MIFC)** and **Value Stream Mapping (VSM)** are familiar visual languages for Kaizen practitioners, with recognisable symbols that practitioners are already comfortable with.
- While the primary users were expected to be Kaizen experts, the product was also intended for logistics hub managers and operational staff without formal Kaizen training, so the tool needed to remain approachable for non-specialists.
...
I explored several wireframe concepts and discussed them with the UX team and frontend developers to balance usability and implementation complexity.

The final design presented validation issues in a prioritized list using collapsible sections. Ordering issues by priority helped users resolve high-priority problems first, often eliminating several secondary issues automatically. Visual warning symbols were used sparingly, as large numbers of icons created an intimidating experience for both developers and end users.
...
<figure>
  <img src="/portfolio-website/case-studies/final-validation.svg" alt="The collapsible validation panel in context within the full application view" class="w-full h-auto" />
...
The mental disconnect between process logic and physical space was a visual problem. The diagram contained too many different element types, and their relationships weren't clear at a glance. Several changes addressed this:
...
The iconography was also inconsistent: some nodes used standard icons, others used abstract Value Stream Mapping–style symbols that many users found difficult to interpret. I worked with other designers to replace these with a unified set - flat 2D icons for process nodes and coloured isometric vector graphics for physical equipment. The visual distinction was deliberate, reinforcing that process blocks represent the concept of an operation rather than its physical counterpart.

<figure>
  <img src="/portfolio-website/case-studies/new-blocks.svg" alt="Evolution from VSM iconography and stock photo physical blocks to a shared design language" class="w-full h-auto" />
  <figcaption class="text-sm text-muted-foreground mt-2">Flat 2D process icons alongside isometric physical layout icons - distinct styles to reinforce the conceptual separation between the two views.</figcaption>
</figure>
...
User testing also revealed that users wanted to reorganize large diagrams more easily. I led the effort to introduce multi-select functionality, allowing multiple nodes to be moved simultaneously. Working with the UX team, we defined mouse and keyboard interactions that felt natural to technical users, including drag-based selection using the left mouse button. Any nodes implicitly connected to the selection were automatically included, preventing accidental partial moves. Selected wires were also given a colour highlight, making them immediately identifiable.

<figure>
  <div class="grid grid-cols-5 gap-4 mt-6">
    <img src="/portfolio-website/case-studies/connection-line.svg" alt="Selected connection wire with blue highlight glow" class="col-span-3 w-full h-auto" />
    <img src="/portfolio-website/case-studies/drag-select.svg" alt="Drag-based multi-select interaction" class="col-span-2 w-full h-auto" />
  </div>
  <figcaption class="text-sm text-muted-foreground mt-2">Left: selected wires now render with a blue highlight glow, floating above other lines for clarity. Right: drag-based multi-select; connected nodes are automatically included in the selection to prevent broken layouts.</figcaption>
</figure>
