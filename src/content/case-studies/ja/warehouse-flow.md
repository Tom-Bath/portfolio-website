...
<figure>
  <img src="/portfolio-website/case-studies/final-validation.svg" alt="アプリケーション全体のビューの中における折りたたみ式バリデーションパネルのデザイン" class="w-full h-auto" />
...
アイコノグラフィーも一貫していませんでした。標準的なアイコンを使うノードもあれば、多くのユーザーが解釈に困った抽象的なVSM風シンボルを使うノードもありました。他のデザイナーと協力してこれらを統一したセットに置き換えました。プロセスノードにはフラットな2Dアイコン、物理機器には彩色アイソメトリックベクターグラフィックを用いました。この視覚的な差異は意図的なもので、プロセスブロックが物理的な実装ではなくオペレーションの概念を表すという認識を強化します。

<figure>
  <img src="/portfolio-website/case-studies/new-blocks.svg" alt="VSMアイコノグラフィーから物理・プロセスアイコン共通デザイン言語への進化" class="w-full h-auto" />
  <figcaption class="text-sm text-muted-foreground mt-2">フラットな2Dのプロセスアイコンとアイソメトリックの物理アイコン。決して同じスタイルにしないことで、二つのビューの概念的な分離を視覚的に強化しています。</figcaption>
</figure>

### ダイアグラムのナビゲーションとマルチセレクト

実際の大規模な倉庫フローは、スケールの核心的な問題を露呉させました。何十ものノードがある状態では、単純な整理作業でさえ簡単ではなくなり、何が選択・編集されているのか追うのが困難になりました。特に問題だったのは、選択された接続ワイヤーに視覚的なインジケーターがなかったことも問題でした。多数のワイヤーが重なり合う大きなダイアグラムでは、手動でたどることなしにどのワイヤーがアクティブなのか判別することができませんでした。

ユーザーテストでも、大きなダイアグラムをより手軽に再整理したいというニーズが明らかになりました。複数のノードを同時に移動できるマルチセレクト機能の導入を主導しました。UXチームと協力して、技術系ユーザーにとって自然に感じられるマウスとキーボードの操作を定義しました。左マウスボタンによるドラッグ選択もその一つです。また、選択されたノードに接続されているノードは自動的に選択に含まれる仕様とし、一部のノードだけが意図せず移動されてレイアウトが壊れることを防ぎました。選択されたワイヤーには色のハイライトが付与され、即座に識別できるようになりました。

<figure>
  <div class="grid grid-cols-5 gap-4 mt-6">
    <img src="/portfolio-website/case-studies/connection-line.svg" alt="選択ワイヤーの青いハイライト表示" class="col-span-3 w-full h-auto" />
    <img src="/portfolio-website/scans/scan-drag-select.webp" alt="ドラッグベースのマルチセレクトインタラクション" class="col-span-2 w-full h-auto" />
  </div>
  <figcaption class="text-sm text-muted-foreground mt-2">左：選択されたワイヤーは青いハイライトグローで表示され、他のラインの上に浮かび上がり明瞭に識別できます。右：ドラッグベースのマルチセレクト。接続されたノードは自動的に選択に含まれ、レイアウトの崩壊を防ぎます。</figcaption>
</figure>
